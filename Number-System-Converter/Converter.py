...
# from tkinter import filedialog, messagebox: Imports specific modules from tkinter:
# filedialog is used for file dialogs (Example: to open a file).
# messagebox is used to display message boxes (Example: error messages).
...
#Window+Widget for Text Finder


def text_finder():

    def extract_characters(filename, start_index, end_index):
        """
        Extracts a substring from the contents of a file.
        ...
        Returns:
            str: The extracted substring, or an error message if the indices are invalid or the file is not found.
        """
        try:
            ...
            with open(filename, 'r') as file:
                ...
                # Extract the substring using the provided indices
    ...
    def browse_file():
        """
        Opens a file dialog to select a file and updates the filename entry.
        """
        # Open a file dialog to select a file
        filename = filedialog.askopenfilename()
        # Insert the selected filename into the entry
        entry_filename.insert(0, filename)

    def extract_text():
        """
        Extracts a substring from the contents of the selected file and displays the result.
        """
        # Get the filename from the entry
        filename = entry_filename.get()
        try:
            # Get the start and end indices from the entries
            start_index = int(entry_start_index.get())
            end_index = int(entry_end_index.get())
        except ValueError:
            # If the indices are not valid integers, display an error message
            messagebox.showerror("Invalid Input", "Please enter valid indices.")
            return

        # Call the extract_characters function with the provided filename and indices
        result = extract_characters(filename, start_index, end_index)
        # Update the result label with the extracted text or error message
        result_label.config(text=f"Result: {result}")

    def clear():
        """
        Clears the entries and the result label.
        """
        # Clear the filename entry
        entry_filename.delete(0, END)
        # Clear the start index entry
        entry_start_index.delete(0, END)
        # Clear the end index entry
        entry_end_index.delete(0, END)
        # Clear the result label
        result_label.config(text="Extracted Characters:")

    # Setting up the GUI

    txtfinder = Tk()  # Create the main window
    txtfinder.title("Text Finder")  # Set the title of the window

    # File selection
    frame_file = Frame(txtfinder)  # Create a frame for the file selection
    frame_file.pack(pady=10)  # Pack the frame with some vertical padding
    Label(frame_file, text="Text File:").pack(side=LEFT)  # Add a label to the frame
    entry_filename = Entry(frame_file, width=50)  # Create an entry field for the file name
    entry_filename.pack(side=LEFT, padx=5)  # Pack the entry field with some horizontal padding
    Button(frame_file, text="Browse", command=browse_file).pack(side=LEFT)  # Add a button to browse for a file

    # Start index input
    frame_start_index = Frame(txtfinder)  # Create a frame for the start index input
    frame_start_index.pack(pady=10)  # Pack the frame with some vertical padding
    Label(frame_start_index, text="Start Index:").pack(side=LEFT)  # Add a label to the frame
    entry_start_index = Entry(frame_start_index, width=10)  # Create an entry field for the start index
    entry_start_index.pack(side=LEFT, padx=5)  # Pack the entry field with some horizontal padding

    # End index input
    frame_end_index = Frame(txtfinder)  # Create a frame for the end index input
    frame_end_index.pack(pady=10)  # Pack the frame with some vertical padding
    Label(frame_end_index, text="End Index:").pack(side=LEFT)  # Add a label to the frame
    entry_end_index = Entry(frame_end_index, width=10)  # Create an entry field for the end index
    entry_end_index.pack(side=LEFT, padx=5)  # Pack the entry field with some horizontal padding

    # Create a frame for the buttons
    frame_buttons = Frame(txtfinder)  # Create a frame for the buttons
    frame_buttons.pack(pady=10)  # Pack the frame with some vertical padding

    # Extract button
    extract_button = Button(frame_buttons, text="Extract", command=extract_text)  # Create a button extract
    extract_button.pack(side=LEFT, padx=5)  # Pack the button with some horizontal padding

    # Clear button
    clear_button = Button(frame_buttons, text="Clear", command=clear)  # Create a button clear
    clear_button.pack(side=LEFT, padx=5)  # Pack the button with some horizontal padding

    # Result label
    result_label = Label(txtfinder, text="Extracted characters: ") # Create a label for result
    result_label.pack(pady=10)  # Pack the label with some vertical padding

    txtfinder.mainloop()  # Start the main event loop
...
submenu.add_separator()
submenu.add_cascade(label='Text Finder',command=text_finder)
submenu.add_separator()
